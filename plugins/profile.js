	...
	commands: {
		...
		profile: {
			...
			async action(message) {
				...
				return this.replyHTML(`<div style="width:100%;overflow-x:auto;display:inline-flex;">${cols.map(col => `<div style="margin:auto 5px auto 0px;${this.pm && !col.startsWith('<img') ? 'min-width:150px;max-width:150px;' : ''}">${col}</div>`).join('')}</div>`);
			},
		},
	},
};
